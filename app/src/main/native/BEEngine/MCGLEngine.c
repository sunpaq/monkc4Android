...
utility(MCGLEngine, GLuint, prepareShader, GLuint Id, const char* vcode, const char* fcode)
{
    ...
    if(MCGLEngine_compileShader(&vertShader, GL_VERTEX_SHADER, vcode)) {
        error_log("compile vertex shader failed");
        exit(0);
    }
    if(MCGLEngine_compileShader(&fragShader, GL_FRAGMENT_SHADER, fcode)) {
        error_log("compile fragment shader failed");
        exit(0);
    }
...
//    }
    ...
    if (MCGLEngine_linkProgram(Id) == 0) {
        error_log("Failed to link program: %d exit\n", Id);
...
utility(MCGLEngine, void, checkGLError, MCFuncPtr callback)
{
    ...
    if( err != GL_NO_ERROR) {
        ...
        if(err == GL_INVALID_OPERATION)
        ...
        else if(err == GL_INVALID_INDEX)
            info = "GL_INVALID_INDEX";
            ...
            char buff[1024];
            info = sprintf(buff, "Unknown Error:%d", err);
        }

        error_log("shader program validate failed %s\n", info);
        exit(0);
    } else {
        callback();
    }
